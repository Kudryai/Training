class Matrix:
    def __init__(self, rows_or_lst, cols=0, fill_value=0):
        if type(rows_or_lst) == list:
            ...
            if not all(len(r) == self._cols for r in rows_or_lst) or not all(
                self.__is_digit(x) for row in rows_or_lst for x in row
            ):
                raise TypeError(
            ...
            if (
                type(rows_or_lst) != int
                or type(cols) != int
                or type(fill_value) not in (int, float)
            ):
                raise TypeError(
                    "аргументы rows, cols - целые числа; fill_value - произвольное число"
                )
    ...
    def __check_indx(self, index):
        r, c = index
        if not (0 <= r < self._rows) or not (0 <= c < self._cols):
            raise IndexError("недопустимые значения индекса")
