class ListMath:
    ...
    def __init__(self, lst=None):
        if lst:
            self.lst = [i for i in lst if type(i) in (int, float)]
        else:
            self.lst = []
    ...
    def __iadd__(self, other):
        return ListMath([i + other for i in self.lst])
    ...
    def __rsub__(self, other):
        return ListMath([other - i for i in self.lst])
